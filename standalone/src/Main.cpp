...
namespace Config {
  constexpr char standaloneName[] = "MultiOledLauncher";
...
std::unique_ptr<dotname::MultiOled> uniqueLib;
...
int processArguments (int argc, const char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::MultiOled> ();
      uniqueLib = std::make_unique<dotname::MultiOled> (Config::assetsPath);
    } else {
